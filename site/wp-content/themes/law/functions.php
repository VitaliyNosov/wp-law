...
// Подключаем наш файл рассширения.
...
function law_scripts() {
	...
	wp_script_add_data('law-respond-js', 'conditional', 'lt IE 9'); // Возможность подключать
	// скрипты по условию. Подробнее в документации. 
  ...
  wp_enqueue_script( 'jquery' );
	...
	// Функция для использования ajax функции в нашем файле java script для асинхронной отправки формы.
...
// Подключаем наш файл tgm Это плагин для уведомления пользователя темы
// какое плагины нужны для работы. Этот файл это измененая копия файла example.php который идет из
// коробки. Подробнее:  https://github.com/TGMPA/TGM-Plugin-Activation


require_once get_template_directory() . '/inc/tgm/tgm.php';


// Подключаем наши коментарии данная функция взата с темы коорая идет по умолчаниюс wordpress
...
// Функция которая вызывает кастомайзер темы для фрейворка Unison
